...
bool CHexEdit::IsHexConvertableText(LPTSTR _text)
{
	...
	if(_tcslen(_text)>=2)
	{
		if(_text[0]==_T('0')&&(_text[1]==_T('x')||_text[1]==_T('X')))
		{
			if((second==_T('x')||second==_T('X'))&&(!(start==0&&end>=2)))
				bPaste=false;
			else if(start>0)
				bPaste=false;
			else
	...
	if(_tcslen(_text)>=1)
	{
		if(head==_T('0')&&(_text[0]==_T('x')||_text[0]==_T('X')))
		{
			i++;
		}
		if((_text[0]==_T('x')||_text[0]==_T('X')))
		{
			if(head!=_T('0')&&start==0)
				bPaste=false;
			else if(!(start==1&&end>=1&&head==_T('0')))
				bPaste=false;
		}
	}
	if(!bPaste) return bPaste;
	...
	{
		TCHAR c=_text[i];
		if(!(c>=48&&c<=57||c>=_T('A')&&c<=_T('F')||c>=_T('a')&&c<=_T('f')))
		{
			bPaste=false;
			break;
		}
...
void CHexEdit::FormatClipboard()
{
    ...
    { 
		...
		{
			if(lptstr[i]!=_T('X')&&lptstr[i]!=_T('x'))
		...
		memcpy(lptstrCopy, lptstr,_tcslen(lptstr)*sizeof(TCHAR));
		GlobalUnlock(hglb); 
...
LRESULT CHexEdit::OnConvert(WPARAM wParam, LPARAM lParam)
{
	...
	if(this->m_bHex)
	{
		...
		UINT n=0;
		if(text.Find(_T("0x"))==0)
			_stscanf(text.GetBuffer(0)+2,_T("%x"),&n);
		else
			_stscanf(text.GetBuffer(0),_T("%x"),&n);
		text.Format(_T("%u"),n);
	...
	else
	{
		//convert dec to hex
		UINT n=0;
		_stscanf(text.GetBuffer(0),_T("%d"),&n);
...
unsigned int CHexEdit::GetValue(void)
{
	...
	else
	{
		//convert dec to hex
		unsigned int n=0;
		_stscanf(text.GetBuffer(0),_T("%u"),&n);
		return n;
	}
}
